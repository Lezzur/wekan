...
- [x] **Blocked panel rework (honesty)** — confirmed at the metal that WeKan's activity log records no `addedLabel` events (only create/delete), so a true "blocked since" is unrecoverable. Reframed the panel from "Age" (false precision) to **"Oldest Blocked Card — Idle"** = time since last activity, a *lower bound* on true block duration. Replaced the full-width red-slab sparkline (an artifact of a near-constant series filling the panel) with a light card + threshold-colored left accent and a thin baseline line. Metric help text updated; `pm_blocked_age_seconds` name kept for Grafana compat.
